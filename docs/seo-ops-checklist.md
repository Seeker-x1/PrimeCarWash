...
`http://出張洗車.jp/` と `http://www.出張洗車.jp/` は HTTPS apex へ 308 する。GSC の「ページにリダイレクトがあります」は除外理由であり、**検証しない**。画面から消すなら **インデックス作成 → 削除** でその2本を一時非表示にし、`https://出張洗車.jp/` だけを検査・登録する。
